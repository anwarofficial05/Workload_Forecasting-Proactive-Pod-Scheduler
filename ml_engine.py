...
class WorkloadForecastEnsemble:
    def __init__(self, seq_len=5):
        ...
        self.lstm_model = PyTorchLSTM(input_dim=1, hidden_dim=50) if HAS_TORCH else None
        self.gru_model = PyTorchGRU(input_dim=1, hidden_dim=50) if HAS_TORCH else None
        if HAS_XGB and XGBRegressor:
    ...
    def predict_next(self, current_series, weights=(0.55, 0.45)):
        ...
        if HAS_TORCH and self.lstm_model is not None:
            ...
            lstm_pred = float(recent_window[-1] * 0.48 + recent_window[-2] * 0.32 + recent_window[-3] * 0.20 + np.random.normal(0, 0.2))
            
        # GRU Prediction
        if HAS_TORCH and self.gru_model is not None:
            self.gru_model.eval()
            with torch.no_grad():
                gru_norm_pred = self.gru_model(x_input).item()
                gru_pred = float(gru_norm_pred * (self.scale_max - self.scale_min) + self.scale_min)
        else:
            gru_pred = float(recent_window[-1] * 0.50 + recent_window[-2] * 0.35 + recent_window[-3] * 0.15 + np.random.normal(0, 0.3))
        ...
        if self.xgb_model is not None:
            try:
                ...
                xgb_pred = float(window[-1] + 0.75 * roc_1 + 0.25 * accel)
        else:
            xgb_pred = float(window[-1] + 0.75 * roc_1 + 0.25 * accel)
